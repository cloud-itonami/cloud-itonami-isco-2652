(ns music-practice.advisor
  "MusicOpsAdvisor — proposes a music-practice operation (catalog license
  grant, commission delivery, release) for a track. The advisor is swappable:
  ...
  license policy; `music-practice.governor` is the independent system that
  decides whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :license-track|:deliver-commission|:release-track
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for track " (:track-id request))})
...
(def ^:private system-prompt
  "You are a music-practice advisor for an independent musician/composer.
   Given an operation request (catalog licensing, commission delivery,
   release), propose an :op, an honest :confidence (0.0-1.0), and a
